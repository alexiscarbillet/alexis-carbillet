...
//    gui_log_to_history("Triggered human wins: " + HUMAN_WINS_AGAIN);
    ...
    for (var i = 1; i < players.length; i++) {
      if (players[i].bankroll < 1 && players[i].status != "BUST") other_making_stand = 1;
      break;
    }
    if (other_making_stand < 1) { // should really check to see if bet_level is big and anyone has called...that's taking a stand too...
      if (BET_LEVEL > 70) {
//        gui_log_to_history("  making an angry move bet_level > 70");
        return internal_what_do_x("40:CALL,60:ALLIN");
      }
//      gui_log_to_history("  making an angry move");
      return internal_what_do_x("15:MED,40:SMALL,45:CALL");
    }
    if (HCONF > 75) {
//      gui_log_to_history("  making an angry move without a stand");
      return internal_what_do_x("15:MED,40:SMALL,45:CALL");
    }
  }
  ...
  if (HCONF > 30) {
    ...
    if (BET_LEVEL > 30) {
      if (CALL_LEVEL < 30) return evel(internal_what_do_x("15:SMALL,85:CALL"));
...
//      gui_log_to_history("  making an angry move with great verdict");
      return internal_what_do_x("10:MED,40:SMALL,50:CALL");
    }
  }
  ...
  if (FLUSH_DRAW) {
    ...
    if (ROUND < 5) {
      ...
      if (BET_LEVEL < 50) {
        if (ROUND < 4) return internal_what_do_x("20:MED,40:SMALL,40:CALL");
        if (ROUND < 5) return internal_what_do_x("5:MED,40:SMALL,55:CALL");
      } else {
        if (CALL_LEVEL < 40) return CALL;
        if (ROUND < 4) return internal_what_do_x("29:CALL"); // depends on how good my cards are!!!!!!!!
        if (ROUND < 5) return internal_what_do_x("9:CALL");
      }
    }
...
function internal_what_do_x (q, r) {
  ...
  while (1) {
    ...
    var b = q.indexOf(",", a);
    if (a < 0 || b < 0) {
      // gui_log_to_history("Action: FOLD");
      return FOLD;
    }
    ...
    p += probability;
    if (r <= p) {
      // gui_log_to_history("Action: " + action);
